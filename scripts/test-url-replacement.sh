...
EXPECTED_URL="https://docs.redhat.com/en/documentation/openshift_container_platform/4.20/html-single/disconnected_environments/index#mirroring-image-set-full"

# Run checker (will exit 1 for outdated, but that's expected)
./ocp-doc-checker -url "$INPUT_URL" -json > test5_output.json || true

# Find the 4.20 version URL
ACTUAL_URL=$(jq -r '.newer_versions[] | select(.version=="4.20") | .url' test5_output.json)
